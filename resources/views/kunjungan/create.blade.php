...
<div class="max-w-7xl mx-auto space-y-6" x-data="kunjunganForm()">
    ...
    <div class="flex justify-between items-center bg-white p-3 rounded-xl border border-gray-100 shadow-sm mb-4">
        <div>
            <h1 class="text-lg font-bold text-gray-900">Input Kunjungan Kapal</h1>
            <p class="text-[10px] text-gray-500 font-medium">Lengkapi semua data kunjungan kapal</p>
        </div>
    ...
    <form action="{{ route('kunjungan.store') }}" method="POST">
            ...
            <div class="border-b border-gray-100">
                <nav class="flex overflow-x-auto">
                    <button type="button" @click="currentTab = 1"
                            class="px-4 py-2 text-xs font-semibold border-b-2 transition-all whitespace-nowrap"
                            :class="currentTab === 1 ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'">
                        1. Data Kunjungan
                    </button>
                    <button type="button" @click="currentTab = 2"
                            class="px-4 py-2 text-xs font-semibold border-b-2 transition-all whitespace-nowrap"
                            :class="currentTab === 2 ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'">
                        2. Kedatangan & Keberangkatan
                    </button>
                    <button type="button" @click="currentTab = 3"
                            class="px-4 py-2 text-xs font-semibold border-b-2 transition-all whitespace-nowrap"
                            :class="currentTab === 3 ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'">
                        3. Penumpang & Kendaraan
                    </button>
                    <button type="button" @click="currentTab = 4"
                            class="px-4 py-2 text-xs font-semibold border-b-2 transition-all whitespace-nowrap"
                            :class="currentTab === 4 ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'">
                        4. Data Muatan
                    </button>
                    <button type="button" @click="currentTab = 5"
                            class="px-4 py-2 text-xs font-semibold border-b-2 transition-all whitespace-nowrap"
                            :class="currentTab === 5 ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'">
                        5. Barang B3
                    </button>
                </nav>
            </div>
        ...
        <div x-show="currentTab === 1" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        ...
                        <a href="{{ route('master.jenis-pelayaran.index') }}" target="_blank"
                           class="text-xs text-blue-600 hover:text-blue-800 font-medium">Kelola &rarr;</a>
                    </div>
                ...
                <!-- Nakhoda (fixed autocomplete with API) -->
                <div>
                    <div class="flex items-center justify-between mb-1">
                        ...
                        <button type="button" 
                                onclick="const kId = document.getElementsByName('kapal_id')[0].value; const kNama = document.querySelector('[x-data^=autocomplete] input[type=text]').value; if(!kId){ Swal.fire('Peringatan', 'Pilih kapal terlebih dahulu!', 'warning'); return; } openQuickAddNakhoda(kId, kNama)"
                                class="text-xs text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                            Tambah Nakhoda
                        </button>
            ...
            <div class="flex justify-end">
                <button type="button" @click="currentTab = 2"
                        class="px-4 py-2 text-xs font-medium bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-all">
                    Selanjutnya →
                </button>
            </div>
        </div>
        ...
        <div x-show="currentTab === 2" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-4">
            ...
            <div class="flex justify-between pt-4">
                <button type="button" @click="currentTab = 1"
                        class="px-4 py-2 text-xs font-medium bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-all">
                    ← Sebelumnya
                </button>
                <button type="button" @click="currentTab = 3"
                        class="px-4 py-2 text-xs font-medium bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-all">
        ...
        <div x-show="currentTab === 3" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-4">
        ...
        <!-- Tab 4: Data Muatan -->
        <div x-show="currentTab === 4" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-4">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-sm font-bold text-gray-900 mb-4 border-b pb-1.5">Data Muatan</h3>
        ...
        <!-- Tab 5: Barang B3 -->
        <div x-show="currentTab === 5" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-4">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-sm font-bold text-gray-900 mb-4 border-b pb-1.5">Data Barang B3 (Bahan Berbahaya & Beracun)</h3>
            ...
            <div class="flex justify-between pt-4">
                <button type="button" @click="currentTab = 4"
                        class="px-3 py-1.5 text-xs font-medium bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 transition-all">
                    ← Sebelumnya
                </button>
                <button type="submit"
...
<div id="modal-quick-pelabuhan" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        <div class="flex justify-between items-center mb-4">
            ...
            <button type="button" onclick="closeQuickAddPelabuhan()" class="text-gray-400 hover:text-gray-600 text-xl">&times;</button>
        </div>
        <form id="form-quick-pelabuhan" class="space-y-3">
            ...
            <div class="flex justify-end gap-2 pt-2">
                <button type="button" onclick="closeQuickAddPelabuhan()"
                        class="px-3 py-1.5 text-xs bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">Batal</button>
                <button type="submit"
                        class="px-3 py-1.5 text-xs bg-blue-600 text-white rounded-md hover:bg-blue-700">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modal-quick-nakhoda" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        <div class="flex justify-between items-center mb-4">
            ...
            <button type="button" onclick="closeQuickAddNakhoda()" class="text-gray-400 hover:text-gray-600 text-xl">&times;</button>
        </div>
        <form id="form-quick-nakhoda" class="space-y-3">
            ...
            <div class="flex justify-end gap-2 pt-2">
                <button type="button" onclick="closeQuickAddNakhoda()"
                        class="px-3 py-1.5 text-xs bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">Batal</button>
                <button type="submit"
                        class="px-3 py-1.5 text-xs bg-blue-600 text-white rounded-md hover:bg-blue-700">Simpan</button>
            </div>
...
<div id="modal-quick-b3" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-base font-bold text-gray-900">Tambah Jenis Barang B3 Baru</h3>
            <button type="button" onclick="closeQuickAddB3()" class="text-gray-400 hover:text-gray-600 text-xl">&times;</button>
        </div>
        <form id="form-quick-b3" class="space-y-3">
            ...
            <div class="flex justify-end gap-2 pt-2">
                <button type="button" onclick="closeQuickAddB3()"
                        class="px-3 py-1.5 text-xs bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300">Batal</button>
                <button type="submit"
                        class="px-3 py-1.5 text-xs bg-orange-600 text-white rounded-md hover:bg-orange-700">Simpan</button>
            </div>
        </form>
    </div>
</div>
...
document.getElementById('form-quick-pelabuhan').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (data.success) {
            Alpine.store || true; // force update
            const sel = document.getElementById('pelabuhan_id');
            if (sel) { const opt = new Option(data.data.nama + ' (' + (data.data.kode ?? '-') + ')', data.data.id, true, true); sel.add(opt); }
            this.reset(); closeQuickAddPelabuhan();
        } else {
...
// ===== QUICK ADD NAKHODA =====
function openQuickAddNakhoda(kapalId, kapalNama) { 
    if (kapalId) {
        document.getElementById('quick-nakhoda-kapal-id').value = kapalId;
        document.getElementById('quick-nakhoda-kapal-nama').value = kapalNama;
    }
    document.getElementById('modal-quick-nakhoda').classList.remove('hidden'); 
}
...
document.getElementById('form-quick-nakhoda').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        if (data.success) {
            ...
            if (hiddenInput) hiddenInput.value = data.data.id;
            if (searchInput) searchInput.value = data.data.nama;
            this.reset(); closeQuickAddNakhoda();
        } else {
            const msgs = data.errors ? Object.values(data.errors).flat().join(', ') : (data.message ?? 'Gagal menyimpan.');
            errEl.textContent = msgs; errEl.classList.remove('hidden');
        }
    ...
    try {
        ...
        if (data.success) {
            document.querySelectorAll('select[name$="[barang_b3_id]"]').forEach(sel => {
                sel.add(new Option(data.data.nama + ' (' + (data.data.un_number ?? '-') + ')', data.data.id));
            });
            this.reset(); closeQuickAddB3();
        } else {
            const msgs = data.errors ? Object.values(data.errors).flat().join(', ') : (data.message ?? 'Gagal menyimpan.');
            errEl.textContent = msgs; errEl.classList.remove('hidden');
        }
