...
class NakhodaController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $query = Nakhoda::with('kapal')->select('nakhodas.*');
            ...
            if ($request->filled('kapal_id')) {
                $query->where('kapal_id', $request->integer('kapal_id'));
            }
            ...
            return datatables()->of($query)
                ->addIndexColumn()
                ->addColumn('kapal_nama', function ($row) {
                    return $row->kapal->nama ?? '-';
                })
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<div class="relative inline-block text-left" x-data="{ open: false }" @click.away="open = false" :class="{ \'z-[50]\': open }">
                                ...
                                <div x-show="open"
                                     ...
                                     style="display: none;">
                                    <div class="py-1">
                                        <button type="button" onclick=\'window.dispatchEvent(new CustomEvent("edit-nakhoda", { detail: '.$jsonRow.' }))\' class="group flex items-center w-full px-4 py-2 text-xs text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors">
                                        ...
                                        <form action="'.route('master.nakhoda.destroy', $row->id).'" method="POST" class="inline">
                                            '.csrf_field().'
                                            '.method_field('DELETE').'
        ...
        return view('master.nakhoda.index', compact('kapals'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $nakhoda = Nakhoda::create($validated);

        if ($request->ajax() || $request->wantsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Data nakhoda berhasil ditambahkan.',
                'data' => $nakhoda->load('kapal'),
            ]);
        }
    ...
    public function update(Request $request, Nakhoda $nakhoda)
    {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Data nakhoda berhasil diperbarui.',
                'data' => $nakhoda->load('kapal'),
            ]);
        }
